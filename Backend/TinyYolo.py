...
class TinyYolo(ObjectDetection):

    def __init__(self):
        """
        Creates a ObjectDetection object for the TinyYolo algorithm.
        ...
        print('[TINY YOLO INIT] Initialize Yolo ')

        print('[TINY YOLO INIT] Setting paths ')
        path = os.path.dirname(os.path.abspath(__file__))
        yolo_path = path + '\yolo'
        labels_path = yolo_path + '\obj.names'
        weights_path = yolo_path + '\yolov3-tiny.weights'
        config_path = yolo_path + '\yolov3-tiny.cfg'

        print('[TINY YOLO INIT] Current dir: ', path)
        print('[TINY YOLO INIT] Weights path: ', weights_path)
        print('[TINY YOLO INIT] Config path: ', config_path)
        ...
        print("[TINY YOLO INIT] loading YOLO from disk...")
        self.__net = cv2.dnn.readNetFromDarknet(config_path, weights_path)
        print("[TINY YOLO INIT] loading YOLO finished...")
    ...
    def forward_pass(self, image, conf, thresh, area):
        ...
        print("[TINY YOLO PASS] YOLO took {:.6f} seconds".format(end - start))
